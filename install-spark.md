...
If `brew cask install java` doesn't work or `java -version` gives you an error and a popup:
1) Get the JRE dmg on http://www.oracle.com/technetwork/java/javase/downloads/index.html
2) Install it
3) In your terminal, type: export JAVA_HOME="`/usr/libexec/java_home`"
4) `java -version` should now gives you something like `java version "1.7.0_45"`
5) You can add `export JAVA_HOME` to your .bash_profile
